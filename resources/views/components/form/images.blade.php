...
<div class="form-images" data-tab-group="lang-images">
    ...
    @foreach ($langs as $lang)
        <div class="{{ $lang->code }}-images-content tab-content tab-images-lang-content {{ $loop->first ? 'active' : '' }}"
            ...
            <x-form.images-box :element="$element" :lang="$lang->code" name="header" multiple="single">
            </x-form.images-box>
            <x-form.images-box :element="$element" :lang="$lang->code" name="gallery" multiple="multiple">
            </x-form.images-box>
        </div>
    @endforeach
</div>
